...
소리 재생은 `sound` 노드로 처리한다. `sound` 노드는 대사처럼 멈춰서 기다리지 않고, 배경이나 캐릭터 등장 노드처럼 즉시 실행된 뒤 다음 노드로 넘어간다.

사운드 파일은 먼저 `config.js`의 `ASSET_MANIFEST.sounds`에 등록해야 한다.
...
BGM을 시작하려면 다음처럼 쓴다. `soundType`이 `"bgm"`이면 기본적으로 반복 재생한다.
...
BGM을 멈추려면 `name` 없이 `stop`을 쓸 수 있다.
...
효과음을 한 번 재생하려면 `soundType`을 `"effect"`로 쓴다. `soundType`을 생략하면 효과음으로 처리된다.
